...
**	teklib/src/lua/tests/luaext.c - Demonstrate Lua language extension
**
...
#define DEFAULT_SCRIPT "src/lua/scripts/luaext.lua"
...
static LUACFUNC TINT win_open(lua_State *L)
{
	...
	*pvis = TOpenModule("visual", 0, TNULL);
	if (*pvis == TNULL)
	{
		/* different possibilities for error treatment: */
...
static LUACFUNC TINT win_newtest(lua_State *L)
{
	...
	** because our own class metatable is also available as an upvalue) */
	...
	luaL_newmetatable(L, "Test*");						/* stack: userdata, metatable */
...
static LUACFUNC void initextensions(lua_State *L, struct LuaExecData *e)
{
...
void TEKMain(struct TTask *task)
{
	...
	L = (lua_State *) TOpenModule("lua5", 0, TNULL);
		...
		else
		{
			...
			if (g->file)
			{
				...
				*/
				...
				chunkexec.led_ReadFunc = (lua_Reader) readfunc;			/* read function */
	...
	TSetRetVal(res);
	if (L) TCloseModule((struct TModule *) LUABASE(L));
