...
/*
**	$Id: luaext.c,v 1.1 2006/08/25 02:39:34 tmueller Exp $
**	teklib/src/lua/tests/luaext.c - Demonstrate Lua and threading
**
**	Written by Timm S. Mueller <tmueller at neoscientists.org>
**	See copyright notice in teklib/COPYRIGHT
**
**	TODO: employ luaL_check... functions
*/
...
#define DEFAULT_SCRIPT "src/lua/scripts/dots.lua"
...
test_delay(lua_State *L)
{
	...
	delayt.ttm_Sec = (TINT) delayf;
	delayt.ttm_USec = (TINT) ((delayf - delayt.ttm_Sec) * 1000000);

	TDelay(LUABASE(L)->tlu_TimeRequest, &delayt);
...
test_newdot(lua_State *L)
{
	...
	*pobj = obj;
	...
	TAddTail(&g->objects, (struct TNode *) obj);
...
test_setdotpos(lua_State *L)
{
	...
	if (!lua_isnumber(L, 2))
		luaL_argerror(L, 2, "argument type incorrect");
	if (!lua_isnumber(L, 3))
		luaL_argerror(L, 3, "argument type incorrect");

	if (pobj == TNULL)
		luaL_argerror(L, 1, "bad handle");
	obj = *pobj;
	if (obj)
	{
		TLock(g->lock); /* protect access to the list */
...
static LUACFUNC void
initextensions(lua_State *L, struct LuaExecData *e)
{
	/* Add a set of functions (no class/methods) to the library "test" */
	luaT_addlib(L, "test", test_functions, e->led_UserData);

	/* Add a "dot" class (functions and methods) to the library "test" */
	luaT_addclass(L, "test", CLASSNAME_DOT, dot_functions, dot_methods,
		e->led_UserData);
}
...
static void scriptexecfunc(struct TTask *task)
{
...
static struct ScriptMessage *
script_init(global *g, lua_State *L, TSTRPTR scriptname, TSTRPTR *args)
{
	...
	if (scriptmsg)
	{
		...
		if (scriptmsg->replyport)
		{
			...
			if (scriptmsg->childtask)
			{
				...
				scriptmsg->execdata.led_ChunkName = scriptname;
				/* program name, aka argv[0] */
				scriptmsg->execdata.led_ProgName = scriptname;
				/* argument vector */
				scriptmsg->execdata.led_ArgV = args;
...
static void
script_run(global *g, struct ScriptMessage *scriptmsg)
{
...
static void render_exit(global *g)
{
	object *obj;
...
static void render_main(global *g)
{
	...
	TVisualSetInput(g->visual, 0,
		TITYPE_CLOSE | TITYPE_COOKEDKEY | TITYPE_NEWSIZE);
	delayt.ttm_Sec = 0;
	...
	while (!g->abort)
	{
		...
		{
			if (imsg->timsg_Type == TITYPE_NEWSIZE)
				checksize = TTRUE;
			if (imsg->timsg_Type == TITYPE_CLOSE)
				g->abort = TTRUE;
		...
		while ((nextnode = node->tln_Succ))
		{
			...
			TVisualFRect(g->visual, (TINT) (obj->x * width),
				(TINT) (obj->y * height), dotw, doth, g->pen[obj->penindex]);
			node = nextnode;
		}
...
void TEKMain(struct TTask *task)
{
	...
	if (TUtilBase && TTimeBase && TIOBase && L)
	{
		...
		if (TimeReq)
		{
			...
			args[ARG_FILE] = (TTAG) DEFAULT_SCRIPT;
			args[ARG_ARGS] = TNULL;
			...
			{
				printf("Usage: %s %s\n", argv[0], ARG_TEMPLATE);
				printf("Threaded Lua interpreter test\n");
				printf("-f=FROM      : Lua script to run [%s]\n",
					DEFAULT_SCRIPT);
			...
			else
			{
				if (render_init(g))
				{
					g->file = TOpenFile((TSTRPTR) args[ARG_FILE],
						TFMODE_READONLY, TNULL);
					if (g->file)
					{
						TINT err = LUA_ERRRUN;
						struct ScriptMessage *msg;

						msg = script_init(g, L, (TSTRPTR) args[ARG_FILE],
							(TSTRPTR *) args[ARG_ARGS]);
						if (msg)
						{
							/* read function */
							msg->execdata.led_ReadFunc =
								(lua_Reader) readfunc;
							/* user init */
							msg->execdata.led_InitFunc = initextensions;
							/* userdata to script */
							msg->execdata.led_UserData = g;

							/* set these if you want errors to be printed: */
							msg->execdata.led_IOBase = TIOBase;
							msg->execdata.led_ErrorFH = TErrorFH();
						...
						{
							default:
								printf("*** Initialization/runtime error\n");
								res = 10;
								break;
	...
	TDBPRINTF(5,("bye... return value: %d\n", res));
}
