...
/*
**	$Id: datetest.c,v 1.3 2006/09/10 01:06:45 tmueller Exp $
**	teklib/src/time/tests/datetest.c - Time module unit test
**
...
void test1(void)
{
	...
	TAstroMakeDate(TAstroBase, &d1, 1,1,1978, TNULL);
	jd = TAstroDateToJulian(TAstroBase, &d1);
...
void test13(void)
{
	...
	TAstroMakeDate(TAstroBase, &d, 28, 2, 1900, TNULL);
	jd = TAstroDateToJulian(TAstroBase, &d) + 1;
	TAstroJulianToDMY(TAstroBase, jd, &day, &month, &year);
	printf("the day following the 29.2.1900 was the %d.%d.%d\n",
		day, month, year);
}
...
TINT printmonthascii(TINT m, TINT y)
{
	...
	day = TGetWeekDay(1, m, y);	/* get Weekday */
	day--;
	if(day<0)
		day = 6;

	we = TGetWeekNumber(1, m, y);	/* get Weeknumber */
	m--;
...
void test7(void)
{
	...
	for (y = from; y <= to; ++y)
	{
		for (m = 1; m <= 12; ++m)
		{
			for (d = 1; d <= 31; ++d)
			{
				...
				{
					TINT yd = TDMYToYDay(d,m,y);
					TUINT d2, m2;

					TYDayToDM(yd, y, &d2, &m2);
	...
	TAstroMakeDate(TAstroBase, &d1, 1,1,from, TNULL);
	TAstroMakeDate(TAstroBase, &d2, 31,12,to, TNULL);
	nd = TDiffDate(&d2, &d1, TNULL);
...
void test8(void)
{
	...
	TAstroGetEaster(TAstroBase,-666,&d,&m);
	printf("Easter in the year -666 is the %d.%d.\n",d,m);
	TAstroGetEaster(TAstroBase,33333,&d,&m);
	printf("Easter in the year 33333 is the %d.%d.\n",d,m);
}

/*****************************************************************************/

void test9(TAPTR astro, TDATE *when)
{
	TTAGITEM itags[4];
	TTAGITEM otags[7];
	struct TDateBox db;
	...
	TUnpackDate(when, &db, TDB_DAY|TDB_MONTH|TDB_YEAR);
...
/*****************************************************************************/
void test11(TAPTR astro, TDATE *when)
{
	TTAGITEM tags[19];
	struct TDateBox db;
	struct TDateBox dt[18];

	TUnpackDate(when, &db, TDB_YEAR);
...
void test12(TAPTR astro, TDATE *when)
{
	struct TDTEclipse e;
	struct TDateBox db;
	TUINT y;
	...
	TAstroMakeDate(TAstroBase, when, 15,12,y-1, TNULL); /* to catch the first eclipse */
	TUnpackDate(when, &db, TDB_YEAR|TDB_MONTH|TDB_DAY);
	printf("\nSun Eclipses for %d:  \n",y);
	do{
		...
		TAstroMakeDate(TAstroBase, when, e.event.tdb_Day,e.event.tdb_Month,e.event.tdb_Year, TNULL);
		TUnpackDate(when, &db, TDB_YEAR|TDB_MONTH|TDB_DAY);
...
void TEKMain(struct TTask *task)
{
	...
	TAstroBase = (struct TAstroBase *)
		TOpenModule("astro", 0, TNULL);
	TUtilBase = (struct TUtilBase *)
		TOpenModule("util", 0, TNULL);
	if (TAstroBase && TUtilBase)
	{
		TDATE uni, now;
		...
		TGetUniversalDate(&uni);
		TUnpackDate(&uni, &dbox, TDB_ALL);
		...
		test8();
		test9(TAstroBase, &now);
		test11(TAstroBase, &now);
		test12(TAstroBase, &now);
	}
	else
		printf("*** Could not open date module.\n");

	TCloseModule((struct TModule *) TUtilBase);
	TCloseModule((struct TModule *) TAstroBase);
}
