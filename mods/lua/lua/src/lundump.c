/*
** $Id: lundump.c,v 1.1 2005/05/08 19:15:46 tmueller Exp $
...
static TString* LLoadString (LoadState* S)
{
...
static void LoadLocals (LoadState* S, Proto* f)
{
 ...
 for (i=0; i<n; i++)
 {
  f->locvars[i].varname=LLoadString(S);
...
static void LoadUpvalues (LoadState* S, Proto* f)
{
 ...
 for (i=0; i<n; i++) f->upvalues[i]=LLoadString(S);
}
 ...
 for (i=0; i<n; i++)
 {
  ...
  switch (t)
  {
   ...
   case LUA_TNUMBER:
	...
	break;
   case LUA_TSTRING:
	setsvalue2n(S->L,o,LLoadString(S));
	break;
   default:
...
static Proto* LoadFunction (LoadState* S, TString* p)
{
 ...
 f->source=LLoadString(S); if (f->source==NULL) f->source=p;
