...
TVOID RenderDirectStandard(THNDL *theDisplay);
...
TTASKENTRY TVOID TEKMain(TAPTR task)
{
	...
	if(utilbase && imgpbase && dhbase && timebase)
	{
		...
		theDisplay=TDisSimpleCreateView(dhbase,"teklib display",width,height,depth,TFALSE,fullscreen,dblbuf,resize);
		if(!theDisplay)
		{
		...
		else
		{
			...
			if(depth==8)
			{
				TIMGARGBCOLOR pal[256];
				TINT i;
				TFLOAT r,g,b,rs,gs,bs;
				...
				for(i=0;i<255;i++)
				{
					r+=rs;
					if(r>1.0f) r=0;
					pal[i].r=(TINT)(r*255.0f);

					g+=gs;
					if(g>1.0f) g=0;
					pal[i].g=(TINT)(g*255.0f);

					b+=bs;
					if(b>1.0f) b=0;
					pal[i].b=(TINT)(b*255.0f);
				}
				TDisSetPalette(dhbase,theDisplay,pal,0,0,256);
			}
			...
			while(!abort)
			{
				...
				if(active || redraw)
				{
					...
					RenderDirectStandard(theDisplay);
...
/**************************************************************************
	...
	if the created display is a standard, here we draw some weird pixels
	directly into to the framebuffer
 **************************************************************************/
TVOID RenderDirectStandard(THNDL *theDisplay)
{
	TIMGPICTURE lockimg;
