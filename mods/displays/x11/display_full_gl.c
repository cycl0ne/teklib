...
TMODAPI TVOID dismod_destroy(TMOD_DISMOD *dismod)
{
	if(dismod->theXDisplay)
	{
		...
		{
			XF86VidModeSwitchToMode(dismod->theXDisplay, dismod->theXScreen, &dismod->deskMode);
			XF86VidModeSetViewPort(dismod->theXDisplay, dismod->theXScreen, 0, 0);
		}
...
TBOOL Dismod_ReadProperties( TMOD_DISMOD *dismod )
{
	...
	if(dismod->theXDisplay)
	{
		...
		dismod->defaultdepth=DefaultDepth(dismod->theXDisplay,dismod->theXScreen);

		if(dismod->defaultdepth>=15)
		{
			dismod->mindepth=dismod->defaultdepth;
			dismod->maxdepth=dismod->defaultdepth;

			/* list dismodmodes */
			if(XF86VidModeQueryExtension(dismod->theXDisplay, &event_base, &error_base) == True)
			{
				XF86VidModeGetAllModeLines(dismod->theXDisplay, dismod->theXScreen, &dismod->numdismodes, &xmodes);
				dismod->modelist=(TDISMODE*)TExecAlloc0(dismod->exec,dismod->modelist,dismod->numdismodes*sizeof(TDISMODE));
				for (i = 0; i < dismod->numdismodes; i++)
				{
					dismod->modelist[i].width=xmodes[i]->hdisplay;
					dismod->modelist[i].height=xmodes[i]->vdisplay;
					dismod->modelist[i].depth=dismod->defaultdepth;
				}
				XFree(xmodes);
...
TBOOL Dismod_CreateWindow(TMOD_DISMOD *dismod)
{
	...
	if(dismod->defaultdepth<8 || dismod->defaultdepth!=dismod->depth)
		return TFALSE;

	XF86VidModeGetAllModeLines(dismod->theXDisplay, dismod->theXScreen, &modeNum, &modes);
	/* save desktop-resolution before switching modes */
	dismod->deskMode = *modes[0];
	/* look for mode with requested resolution */
	i=0;
	suc=TFALSE;
	while( i < modeNum && !suc )
	{
		if ((modes[i]->hdisplay == dismod->width) && (modes[i]->vdisplay == dismod->height))
		{
			bestMode = i;
			suc=TTRUE;
		}
	...
	XF86VidModeSwitchToMode(dismod->theXDisplay, dismod->theXScreen, modes[bestMode]);
	XF86VidModeSetViewPort(dismod->theXDisplay, dismod->theXScreen, 0, 0);
	XFree(modes);
	dismod->fullscreen=TTRUE;
	...
	TExecFillMem(dismod->exec,&attr,0,sizeof(XSetWindowAttributes));
	...
	dismod->theXWindow = XCreateWindow(dismod->theXDisplay,
										...
										0, 0, dismod->width, dismod->height,
	...
	XWarpPointer(dismod->theXDisplay,None,dismod->theXWindow,0, 0, 0, 0, 0, 0);
	XMapRaised(dismod->theXDisplay, dismod->theXWindow);
	XGrabKeyboard(dismod->theXDisplay,dismod->theXWindow, True, GrabModeAsync, GrabModeAsync, CurrentTime);
	XGrabPointer(dismod->theXDisplay,dismod->theXWindow, True, ButtonPressMask, GrabModeAsync, GrabModeAsync,dismod->theXWindow, None, CurrentTime);
