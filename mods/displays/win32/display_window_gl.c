...
TBOOL Dismod_CreateWindow(TMOD_DISMOD *dismod)
{
	...
	windowflags=WS_OVERLAPPED | WS_CLIPCHILDREN | WS_CLIPSIBLINGS;
	if(dismod->resize)
	{
...
TVOID Dismod_DestroyWindow(TMOD_DISMOD *dismod)
{
	...
	if( dismod->hwnd )
	{
		...
		SetWindowLong(dismod->hwnd, GWL_USERDATA, 0L);
		DestroyWindow(dismod->hwnd );
		UnregisterClass((LPCTSTR) (TUINT32) dismod->wclass,(HINSTANCE) NULL);
		dismod->hwnd=NULL;
		dismod->window_ready=TFALSE;
	}
}
