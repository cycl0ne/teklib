...
typedef struct _TModDisplay
{
	...
	/* status indicators */
	TBOOL   isMovingTheWindow;
	TBOOL   isClosingApplication;
	TBOOL   isControlActive;
	TBOOL   isResizingTheWindow;
...
TBOOL Display_CreateWindow(TMOD_DISPLAY *display);
...
/**************************************************************************
	tek_init
 **************************************************************************/
TMODENTRY TUINT tek_init_display_window_gl(TAPTR selftask, TMOD_DISPLAY *mod, TUINT16 version, TTAGITEM *tags)
{
	if (!mod)
	{
...
TMODAPI TBOOL display_create(TMOD_DISPLAY *display, TSTRPTR title, TINT x, TINT y, TINT w, TINT h, TINT d, TUINT flags)
{
	...
	display->dblbuf=TTRUE;										/* window mode has always doublebuffers */

	if ((flags & TDISCF_RESIZEABLE) == TDISCF_RESIZEABLE)
		display->resize = TTRUE;
	else
		display->resize = TFALSE;
	...
	attrList[1] = AGL_RED_SIZE;
	attrList[3] = AGL_GREEN_SIZE;
	attrList[5] = AGL_BLUE_SIZE;
	attrList[7] = AGL_DEPTH_SIZE;

	if (display->dblbuf)
	{
		attrList[9] = AGL_DOUBLEBUFFER;
...
//	Display_InstallDrawTimer(display);				// timer is needed for waitmsg() to sync on redraw-event

	display->windowname=TExecAlloc0(display->exec,TNULL,strlen(title)+1);
	strcpy(display->windowname,title);

	if ( Display_CreateWindow (display))
	{
		...
		aglSetDrawable (display->gldisplayContext, GetWindowPort (display->windowRef));
...
TMODAPI TVOID display_destroy(TMOD_DISPLAY *display)
{
	if (display->window_ready)
	{
		...
		DisposeWindow (display->windowRef);
	}
}
...
TMODAPI TVOID display_waitmsg(TMOD_DISPLAY *display)
{
	EventRef		theEvent;
#if 0
	EventTypeSpec   eventTypes[] =					/* maybe useful in later versions... we will see */
	{
...
TBOOL Display_ReadProperties( TMOD_DISPLAY *display )
{
	...
	for (i=0;i<display->numdismodes;i++)
	{
		...
		display->minwidth  = 128;
		display->minheight = 128;
		display->mindepth  = 8;
...
TBOOL Display_CreateWindow(TMOD_DISPLAY *display)
{
	...
	Rect		windowRect = { 0 };
	TUINT32		windowAttrs = kWindowCloseBoxAttribute | kWindowFullZoomAttribute | kWindowCollapseBoxAttribute;
	
	InitCursor( );
	
	display->windowRect.origin.x = 0;
	display->windowRect.origin.y = 0;
	
	display->windowRect.size.width = display->width;
	display->windowRect.size.height = display->height;
	
	SetRect( &windowRect, display->windowRect.origin.x, display->windowRect.origin.y, display->width, display->height);

	if ( display->resize == TTRUE )
		windowAttrs |= kWindowResizableAttribute;
	
	if (CreateNewWindow( kDocumentWindowClass, windowAttrs, &windowRect, &display->windowRef ) != noErr)
		return TFALSE;
	
	windowTitle = CFStringCreateWithCString (NULL, display->windowname, CFStringGetSystemEncoding());
	SetWindowTitleWithCFString (display->windowRef, windowTitle);

	display->window_ready=TTRUE;

	SetPortWindowPort (display->windowRef);
	ShowWindow( display->windowRef );
...
TVOID processMouseEvents( EventRef theEvent, TAPTR mod )
{
	...
	switch (eventKind)
	{
		case kEventMouseDown:
		{
			...
			// testen, ob mauseklick innerhalb der titelzeile oder einem control passiert ist
			{
				Rect dragRgn;
				Rect closeboxRgn;
				Rect zoomboxRgn;
				...
				GetEventParameter ( theEvent, kEventParamMouseLocation, typeQDPoint, TNULL, sizeof(Point), TNULL, &mouseLoc );

				GetWindowBounds ( display->windowRef, kWindowTitleBarRgn, &dragRgn);
				GetWindowBounds ( display->windowRef, kWindowCloseBoxRgn, &closeboxRgn);
				GetWindowBounds ( display->windowRef, kWindowZoomBoxRgn, &zoomboxRgn);
				GetWindowBounds ( display->windowRef, kWindowCollapseBoxRgn, &collapseboxRgn);
				GetWindowBounds ( display->windowRef, kWindowGrowRgn, &growRgn);
				
				if ( PtInRect ( mouseLoc, &closeboxRgn) == TTRUE )
					display->isClosingApplication = TTRUE;
				else if ( PtInRect ( mouseLoc, &zoomboxRgn) == TTRUE );
//					printf("zoombox clicked\n");
				else if ( PtInRect ( mouseLoc, &collapseboxRgn) == TTRUE );
//					printf("collapsebox clicked\n");
				else if ( PtInRect ( mouseLoc, &growRgn) == TTRUE )
					display->isResizingTheWindow = TTRUE;
				else if ( PtInRect ( mouseLoc, &dragRgn) == TTRUE )
					display->isMovingTheWindow = TTRUE;
		...
		case kEventMouseUp:
		{
			...
			/* does the click happened in a window control such as titlebar,zoom,...? */
			{
				Rect dragRgn;
				Rect closeboxRgn;
				Rect zoomboxRgn;
				...
				if ( PtInRect ( mouseLoc, &closeboxRgn) == TTRUE )
				{
					if ( display->isClosingApplication == TTRUE )
					{
						display->msgcode = TDISMSG_CLOSE;
					}
				}
				else if ( PtInRect ( mouseLoc, &zoomboxRgn) == TTRUE );
/*					printf("zoombox released\n"); */
				else if ( PtInRect ( mouseLoc, &collapseboxRgn) == TTRUE );
/*					printf("collapsebox released\n"); */

				display->isResizingTheWindow = TFALSE;
				display->isMovingTheWindow = TFALSE;
		...
		{
			Point mouseLoc = { 0 };			// obtaining start point of the drag
			GetEventParameter ( theEvent, kEventParamMouseLocation, typeQDPoint, TNULL, sizeof(Point), TNULL, &mouseLoc );
			
			if ( display->isControlActive == TFALSE && display->isResizingTheWindow == TFALSE && display->isMovingTheWindow == TTRUE )
			{
				Rect		screenRect = { 0 };
				RgnHandle   screenRgn = TNULL;

				GetAvailableWindowPositioningRegion(TNULL, screenRgn);
				RectRgn(screenRgn, &screenRect);

				DragWindow(display->windowRef, mouseLoc, &screenRect);
				aglUpdateContext (display->gldisplayContext);
				
				
				display->msgcode = TDISMSG_MOVE;
			}
			else if ( display->isControlActive == TFALSE && display->isResizingTheWindow == TTRUE )				// RESIZE
			{
				Rect	newSize = { 0 };
				Rect	windowRect = { 0 };
				TUINT32 newwidth = 0,
				...
				ResizeWindow(display->windowRef, mouseLoc, TNULL, &newSize);
				
				newwidth = newSize.right - newSize.left;
				newheight = newSize.bottom - newSize.top;
				
				display->msgcode = TDISMSG_RESIZE;
	
				display->drect.x = windowRect.left;
				display->drect.y = windowRect.top;
				display->drect.width = newwidth;
				display->drect.height = newheight;
				
				display->width = display->windowRect.size.width = newwidth;
				display->height = display->windowRect.size.height = newheight;
				display->bytesperrow = display->width * display->bufferpixelsize;
