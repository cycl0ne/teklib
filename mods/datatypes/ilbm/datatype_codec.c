...
*/
TMODENTRY TUINT tek_init_datatype_codec_ilbm(TAPTR selftask, TMOD_DTCODEC *mod, TUINT16 version, TTAGITEM *tags)
{
	if (!mod)
	{
...
static TCALLBACK TMOD_DTCODEC *mod_open(TMOD_DTCODEC *dtcodec, TAPTR selftask, TTAGITEM *tags)
{
	...
	if(fp)
	{
		...
		if (dtcodec)
		{
			...
			dtcodec->fp=fp;
			dtcodec->read=read_ilbm_open(dtcodec);
			if(dtcodec->read)
				return dtcodec;
		}
	}
...
TMODAPI TTAGITEM* dtcodec_getattrs(TMOD_DTCODEC *dtcodec)
{
	if(dtcodec->read)
	{
		...
		TSetTag(dtcodec->attribut_taglist[1], TDTAG_FULLNAME, "ILBM Picture");
		TSetTag(dtcodec->attribut_taglist[2], TDTAG_NUMSUBCLASSES, 0);

		/* fill with class-typical stuff */
		TSetTag(dtcodec->attribut_taglist[3], TDTAG_PICWIDTH, (TINT)dtcodec->width);
		TSetTag(dtcodec->attribut_taglist[4], TDTAG_PICHEIGHT, (TINT)dtcodec->height);
		...
		TSetTag(dtcodec->attribut_taglist[7], TDTAG_PICPIXELWIDTH, dtcodec->pix_w);
		TSetTag(dtcodec->attribut_taglist[8], TDTAG_PICPIXELHEIGHT, dtcodec->pix_h);
...
TMODAPI TBOOL dtcodec_domethod(TMOD_DTCODEC *dtcodec, TTAGITEM *taglist)
{
	...
	if(dtcodec->read)
	{
		...
		if(data)
			suc=read_ilbm_data(dtcodec,data);

		if(dtcodec->format==IMGFMT_CLUT || dtcodec->format==IMGFMT_PLANAR)
		{
			if(palette && suc)
				suc=read_ilbm_palette(dtcodec,palette);
		}
	}
	return suc;
}
