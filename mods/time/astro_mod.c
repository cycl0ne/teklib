...
/*
**	$Id: astro_mod.c,v 1.17 2005/09/13 02:42:26 tmueller Exp $
**	teklib/mods/time/astro_mod.c - Astronomical calculations module
**
...
#define AE 149598000	/* Astronomical Unit, median distance Sun - Earth */	
...
static TCALLBACK TVOID mod_destroy(TMOD_ASTRO *mod);
static TCALLBACK TMOD_ASTRO *mod_open(TMOD_ASTRO *mod, TAPTR task,
	TTAGITEM *tags);
static TCALLBACK TVOID mod_close(TMOD_ASTRO *mod, TAPTR task);

EXPORT TVOID dt_geteaster(TMOD_ASTRO *astro, TINT y,TINT *d, TINT *m );
EXPORT TINT dt_getastrofacts(TMOD_ASTRO *astro, TTAGITEM *in, TTAGITEM *out);
EXPORT TVOID dt_getsunpos(TMOD_ASTRO *astro, TDOUBLE T, TDOUBLE *Ra,
	TDOUBLE *Dec, TDOUBLE *di);
EXPORT TVOID dt_objectriseset(TMOD_ASTRO *astro, TDOUBLE T, TDOUBLE Ra,
	TDOUBLE Dec, TFLOAT l, TFLOAT b, TINT ob, TFLOAT *rt, TFLOAT *st);
EXPORT TVOID dt_converttohms(TMOD_ASTRO *astro, TFLOAT t, TINT *h, TINT *m,
	TINT *s);
EXPORT TVOID dt_getmoonpos(TMOD_ASTRO *astro, TDOUBLE T, TDOUBLE *Ra,
	TDOUBLE *Dec, TDOUBLE *di);
EXPORT TVOID dt_getflexevent(TMOD_ASTRO *astro, TINT y, TTAGITEM *events);
EXPORT TVOID dt_getphases(TMOD_ASTRO *astro,struct TDateBox *db, struct TDTMoonPhase *phase);	
EXPORT TVOID dt_nexteclipse(TMOD_ASTRO *astro,struct TDateBox *db, struct TDTEclipse *eclipse, TINT mode);	
...
TMODENTRY TUINT 
tek_init_astro(TAPTR task, TMOD_ASTRO *astro, TUINT16 version, TTAGITEM *tags)
{
	if (astro == TNULL)
	{
		if (version == 0xffff)
	...
	if (astro->lock)
	{
		...
		astro->mod.tmd_OpenFunc = (TMODOPENFUNC) mod_open;
		astro->mod.tmd_CloseFunc = (TMODCLOSEFUNC) mod_close;
		astro->mod.tmd_DestroyFunc = (TDFUNC) mod_destroy;
		astro->mod.tmd_Flags |= TMODF_EXTENDED;
...
static TCALLBACK TMOD_ASTRO *
mod_open(TMOD_ASTRO *astro, TAPTR task, TTAGITEM *tags)
{
	TMOD_ASTRO *result = astro;

	TExecLock(TExecBase, astro->lock);
	if (astro->time)
	{
		astro->refcount++;
	}
	...
	{
		astro->time = TExecOpenModule(TExecBase, "time", 0, TNULL);
		if (astro->time)
		{
			astro->refcount = 1;
		}
	...
	TExecUnlock(TExecBase, astro->lock);

	return result;
}

static TCALLBACK TVOID
mod_close(TMOD_ASTRO *astro, TAPTR task)
{
	TExecLock(TExecBase, astro->lock);
	if (astro->time)
	{
		if (--astro->refcount == 0)
		{
			TExecCloseModule(TExecBase, astro->time);
		}
	}
	TExecUnlock(TExecBase, astro->lock);
}
...
mod_vectors[MOD_NUMVECTORS] =
{
	...
	(TAPTR) TNULL,
	(TAPTR) dt_geteaster,
	(TAPTR) dt_getastrofacts,
	(TAPTR) dt_getsunpos,
	(TAPTR) dt_objectriseset,
	(TAPTR) dt_converttohms,
	(TAPTR) dt_getmoonpos,
	(TAPTR) dt_getflexevent,
	(TAPTR) dt_getphases,
	(TAPTR) dt_nexteclipse,
};
...
EXPORT TVOID 
dt_geteaster(TMOD_ASTRO *astro, TINT y, TINT *dd, TINT *mm)
{
...
TVOID dt_subaddeaster(TMOD_ASTRO *astro, TDATE *easter, TINT diff, struct TDateBox *db, TINT mode)
{
	TDATE 	tmp;
	
	tmp = *easter;
	if(mode)
		TTimeAddDate(astro->time, &tmp, diff, TNULL);
	else
		TTimeSubDate(astro->time, &tmp, diff, TNULL);
	
	TTimeUnpackDate(astro->time, &tmp, db, TDB_WDAY|TDB_DAY|TDB_MONTH|TDB_YEAR);
}
/*****************************************************************************/
/* 
...
*/
EXPORT TVOID
dt_getflexevent(TMOD_ASTRO *astro, TINT y, TTAGITEM *events)
{
	struct TDateBox *db;
	TDATE	easter;
	TDATE 	tmp;
	...
	TTimeMakeDate(astro->time, &easter, d,m,y, TNULL);

	/* get Easter */
	if((db = (struct TDateBox *) TGetTag(events, TEVT_Easter, TNULL)))
		TTimeUnpackDate(astro->time, &easter, db, TDB_WDAY|TDB_DAY|TDB_MONTH|TDB_YEAR);
	...
	{
		TINT d;
		TTimeMakeDate(astro->time, &tmp, 26,11,y, TNULL);
		d = 7-TTimeGetWeekDay(astro->time, 26,  11, y);
		TTimeAddDate(astro->time, &tmp, d, TNULL);
		TTimeUnpackDate(astro->time, &tmp, db, TDB_WDAY|TDB_DAY|TDB_MONTH|TDB_YEAR);
	}
	
	/* get Penance (Buß- und Bettag) */
	if((db = (struct TDateBox *) TGetTag(events, TEVT_Penance, TNULL)))
	{
		TINT d;
		TTimeMakeDate(astro->time, &tmp, 26,11,y, TNULL);
		d = 4+TTimeGetWeekDay(astro->time, 26,  11, y);
		TTimeSubDate(astro->time, &tmp, d, TNULL);
		TTimeUnpackDate(astro->time, &tmp, db, TDB_WDAY|TDB_DAY|TDB_MONTH|TDB_YEAR);
	}
	...
	if((db = (struct TDateBox *) TGetTag(events, TEVT_DeadSun, TNULL)))
	{
		TINT d;
		TTimeMakeDate(astro->time, &tmp, 26,11,y, TNULL);
		d = TTimeGetWeekDay(astro->time, 26,  11, y);
		TTimeSubDate(astro->time, &tmp, d, TNULL);
		TTimeUnpackDate(astro->time, &tmp, db, TDB_WDAY|TDB_DAY|TDB_MONTH|TDB_YEAR);
	}
	...
	{
		TINT d;
		TTimeMakeDate(astro->time, &tmp, 1,5,y, TNULL);
		d = TTimeGetWeekDay(astro->time, 1,  5, y);
		if(d==0)
			d = 7;
		else
			d = 14-d;
	...
	{
		TINT d;
		TTimeMakeDate(astro->time, &tmp, 1,10,y, TNULL);
		d = TTimeGetWeekDay(astro->time, 1,  10, y);
		if(d!=0)
			d = 7-d;
	...
	if((db = (struct TDateBox *) TGetTag(events, TEVT_BeginDST, TNULL)))
	{
		...
		{
			TTimeMakeDate(astro->time, &tmp, 31,3,y, TNULL);
			d = TTimeGetWeekDay(astro->time, 31,  3, y);
			TTimeSubDate(astro->time, &tmp, d, TNULL);
		}
	...
	if((db = (struct TDateBox *) TGetTag(events, TEVT_EndDST, TNULL)))
	{
		...
		if(db->tdb_Year<1996)
		{
			TTimeMakeDate(astro->time, &tmp, 30,9,y, TNULL);
			d = TTimeGetWeekDay(astro->time, 30,  9, y);
			TTimeSubDate(astro->time, &tmp, d, TNULL);
		}else
		{
			TTimeMakeDate(astro->time, &tmp, 31,10,y, TNULL);
			d = TTimeGetWeekDay(astro->time, 31, 10, y);
			TTimeSubDate(astro->time, &tmp, d, TNULL);
		}
...
EXPORT TINT 
dt_getastrofacts(TMOD_ASTRO *astro, TTAGITEM *input, TTAGITEM *output)
{
	TINT d,m,y,*pi;
	...
	jd = TTimeMYToJulian(astro->time, m, y) + d;
...
EXPORT TVOID
dt_getsunpos(TMOD_ASTRO *astro, TDOUBLE T,TDOUBLE *Ra, TDOUBLE *Dec,
	TDOUBLE *di)
{
	TDOUBLE L0,MA,MAr,e,C,slon,v,U,e0;
	
	
	L0=280.46645+36000.76983*T+0.0003032*(T*T);  /*mittlere Länge der Sonne */
...
EXPORT TVOID 
dt_objectriseset(TMOD_ASTRO *astro, TDOUBLE T, TDOUBLE Ra, TDOUBLE Dec,
...
EXPORT TVOID 
dt_converttohms(TMOD_ASTRO *astro, TFLOAT t, TINT *h, TINT *m, TINT *s)
{
	*h = (int)t;
	*m = (int)((t - *h)*60);
	*s = (int)((((t - *h) * 60) - *m) * 60);
...
EXPORT TVOID 
dt_getmoonpos(TMOD_ASTRO *astro, TDOUBLE T,TDOUBLE *Ra, TDOUBLE *Dec,
	TDOUBLE *di)
{
...
EXPORT TVOID 
dt_getphases(TMOD_ASTRO *astro, struct TDateBox *db,struct TDTMoonPhase *ph)
{
	...
	T = k/1236.85;
	jd = TTimeMYToJulian(astro->time,(TINT)m,(TINT)j);
	jd=(jd-2451545)/36525;	 		/* Zeit in Julianischen Jahrhunderten */
	/* newmoon */
	JDE = dt_calcphase(k,jd,0);
	TTimeJulianToDMY(astro->time,JDE,&D,&M,&J);
	TTimeMakeDate(astro->time,&mdate,D,M,J,TNULL);
	TTimeUnpackDate(astro->time, &mdate, &ph->new, TDB_WDAY|TDB_DAY|TDB_MONTH|TDB_YEAR);
	
	/* first quater */
	JDE = dt_calcphase(k,jd,1);
	TTimeJulianToDMY(astro->time,JDE,&D,&M,&J);
	TTimeMakeDate(astro->time,&mdate,D,M,J,TNULL);
	TTimeUnpackDate(astro->time, &mdate, &ph->first, TDB_WDAY|TDB_DAY|TDB_MONTH|TDB_YEAR);

	/* fullmoon */
	JDE = dt_calcphase(k,jd,2);
	TTimeJulianToDMY(astro->time,JDE,&D,&M,&J);
	TTimeMakeDate(astro->time,&mdate,D,M,J,TNULL);
	TTimeUnpackDate(astro->time, &mdate, &ph->full, TDB_WDAY|TDB_DAY|TDB_MONTH|TDB_YEAR);

	/* last quater */
	JDE = dt_calcphase(k,jd,3);
	TTimeJulianToDMY(astro->time,JDE,&D,&M,&J);
	TTimeMakeDate(astro->time,&mdate,D,M,J,TNULL);
	TTimeUnpackDate(astro->time, &mdate, &ph->last, TDB_WDAY|TDB_DAY|TDB_MONTH|TDB_YEAR);
}

TBOOL
dt_eclipse(TMOD_ASTRO *astro, TDOUBLE k, TDOUBLE ts, struct TDTEclipse *eclipse, TINT mode)
{
	...
	TTimeJulianToDate(astro->time, jde, &date);
	TTimeUnpackDate(astro->time, &date, &eclipse->event, TDB_ALL);
	return TTRUE;

}

EXPORT TVOID
dt_nexteclipse(TMOD_ASTRO *astro,struct TDateBox *db, struct TDTEclipse *eclipse, TINT mode)
{
	TDOUBLE j,m,k,tmp,d;
	TDOUBLE behind,jd;
	
	d = (TDOUBLE)TTimeDMYToYDay(astro->time,db->tdb_Day,db->tdb_Month,db->tdb_Year);
...
**	Revision History
**	$Log: astro_mod.c,v $
**	Revision 1.17  2005/09/13 02:42:26  tmueller
**	updated copyright reference
**	
**	Revision 1.16  2005/09/10 22:52:06  tmueller
**	cosmetic
**	
**	Revision 1.15  2005/09/08 00:02:38  tmueller
**	API extended
**	
**	Revision 1.14  2004/08/01 11:33:05  tmueller
**	few compiler glitches fixed
**	
**	Revision 1.13  2004/05/24 17:34:42  fpagels
**	some minor fixes, add pos angle of moon illumination
**	
**	Revision 1.12  2004/05/08 16:51:22  fpagels
**	fix a degree <-> rad conversions bug
**	
**	Revision 1.11  2004/05/08 07:58:27  fpagels
**	add Sun and Moon Eclipse calculation
**	
**	Revision 1.10  2004/05/03 10:20:14  fpagels
**	add calculation of moonphases
**	
**	Revision 1.9  2004/05/01 20:55:22  fpagels
**	add Totensonntag, Begin End Daylight save time
**	
**	Revision 1.8  2004/05/01 14:43:59  fpagels
**	add illuminated part of the moon calculation
**	
**	Revision 1.7  2004/04/25 08:57:00  fpagels
**	add Laetare- and Trinity Sunday and cleanup
**	
**	Revision 1.6  2004/04/25 07:29:39  fpagels
**	add some flex holidays and events
**	
**	Revision 1.5  2004/04/18 14:14:51  tmueller
**	TTAG changed from TAPTR to TUINTPTR; atomdata changed from TAPTR to TTAG
**	
**	Revision 1.4  2004/01/08 16:58:23  tmueller
**	Please remember to NOT USE printf() in modules, use tdbprintf() instead.
**	
**	Revision 1.3  2003/12/21 13:49:33  fpagels
**	moon sun/rise was nonsens
**	
**	Revision 1.2  2003/12/12 15:03:57  tmueller
**	Updated minor version number
**	
**	Revision 1.1.1.1  2003/12/11 07:19:18  tmueller
**	Krypton import
**	
*/
