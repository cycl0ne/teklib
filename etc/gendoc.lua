	...
	for line in f:lines() do
		...
		while true do
			...
			if not classname then
				...
				elseif state.ext == "idf" then
					local n = line:match("^%.name%s+(%S+)%s*.*$")
					if n then
						classname = n
						shortname = n
					end
